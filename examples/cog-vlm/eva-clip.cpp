...
#define TN_POS_EMBD "%s.position_embd.weight" // ok
#define TN_CLASS_EMBD "v.class_embd" // ok
#define TN_PATCH_EMBD_W "v.patch_embd.weight" // ok
#define TN_PATCH_EMBD_B "v.patch_embd.bias" // ok
#define TN_ATTN_K "%s.blk.%d.attn_k.%s" // ok
#define TN_ATTN_Q "%s.blk.%d.attn_q.%s" // ok
#define TN_ATTN_V "%s.blk.%d.attn_v.%s" // ok
#define TN_ATTN_OUTPUT "%s.blk.%d.attn_out.%s"
#define TN_FFN_DOWN "%s.blk.%d.ffn_down.%s" // ok
#define TN_FFN_UP "%s.blk.%d.ffn_up.%s" // ok
#define TN_LN_1 "%s.blk.%d.ln1.%s" // ok
#define TN_LN_2 "%s.blk.%d.ln2.%s"
#define TN_LINEAR_PROJ "v.linear_proj.weight"
#define TN_DENSE_1 "v.dense_h_to_4h.weight"
#define TN_DENSE_2 "v.dense_4h_to_h.weight"
#define TN_GATE_PROJ "v.gate_proj.weight"
#define TN_PROJ_NORM "v.norm1.%s"
#define TN_BOI_TOKEN "v.boi"
#define TN_EOI_TOKEN "v.eoi"
...
struct clip_vision_model {
    ...
    // COG-VLM projection
    struct ggml_tensor * linear_proj;
    struct ggml_tensor * dense_h_to_4h;
    struct ggml_tensor * gate_proj;
    struct ggml_tensor * dense_4h_to_h;
    struct ggml_tensor * norm1_w;
    struct ggml_tensor * norm1_b;

    struct ggml_tensor * boi_token;
    struct ggml_tensor * eoi_token;
};
...
static ggml_cgraph * clip_image_build_graph(clip_ctx * ctx, const clip_image_f32_batch * imgs) {
    ...
    for (int il = 0; il < n_layer; il++) {
        ...
        // self-attention
        {
            ...
            // * should be: d_head*n_head x num_positions x batch

            Q = ggml_cont(ctx0, ggml_permute(ctx0, Q, 0, 2, 1, 3));
            Q = ggml_reshape_3d(ctx0, Q, d_head, num_positions, n_head * batch_size);
            // * should be: d_head x num_positions x n_head x batch
            ...
            V = ggml_reshape_4d(ctx0, V, num_positions, d_head, n_head, batch_size);
            // * should be: num_positions x d_head x n_head x batch

            struct ggml_tensor * KQ = ggml_mul_mat(ctx0, K, Q);
            // * should be: num_positions x num_positions x n_head x batch
            KQ = ggml_soft_max_inplace(ctx0, KQ);
            struct ggml_tensor * KQV = ggml_mul_mat(ctx0, V, KQ);
            // * should be: num_positions x num_positions x n_head x batch
            KQV = ggml_cont(ctx0, ggml_permute(ctx0, KQV, 0, 2, 1, 3));
            // * should be: d_head x n_head  x num_positions x batch

            cur = ggml_cpy(ctx0, KQV, ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, hidden_size, num_positions, batch_size));
        }

        // * cur should be: d_head *n_head x num_positions x batch x 1
    ...
    // glu projector
    {
        ...
        // GLU projection
        embeddings = ggml_mul_mat(ctx0, model.linear_proj, embeddings);

        embeddings = ggml_norm(ctx0, embeddings, eps);
        embeddings = ggml_add(ctx0, ggml_mul(ctx0, embeddings, model.norm1_w), model.norm1_b);

        embeddings = ggml_gelu(ctx0, embeddings);

        struct ggml_tensor * gate = ggml_silu(ctx0, ggml_mul_mat(ctx0, model.gate_proj, embeddings));
        struct ggml_tensor * act = ggml_mul_mat(ctx0, model.dense_h_to_4h, embeddings);
        embeddings = ggml_mul(ctx0, gate, act);

        embeddings = ggml_mul_mat(ctx0, model.dense_4h_to_h, embeddings);
    }

    // add BOI/EOI tokens
    {
        struct ggml_tensor * embeddings_tokens = ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, model.boi_token->ne[0], num_patches + 2, batch_size);
        ggml_allocr_alloc(ctx->compute_alloc, embeddings_tokens);
        if (!ggml_allocr_is_measure(ctx->compute_alloc)) {
            void* zero_mem = malloc(ggml_nbytes(embeddings_tokens));
            memset(zero_mem, 0, ggml_nbytes(embeddings_tokens));
            ggml_backend_tensor_set(embeddings_tokens, zero_mem, 0, ggml_nbytes(embeddings_tokens));
            free(zero_mem);
        }

        embeddings_tokens = ggml_acc(ctx0, embeddings_tokens, model.boi_token,
                                     embeddings_tokens->nb[1], embeddings_tokens->nb[2], embeddings_tokens->nb[3], 0);
        embeddings_tokens = ggml_acc(ctx0, embeddings_tokens, embeddings,
...
struct clip_ctx * clip_model_load(const char * fname, const int verbosity = 1) {
    ...
    if (new_clip->has_vision_encoder) {
        ...
        vision_model.patch_embeddings_w   = get_tensor(new_clip->ctx_data, TN_PATCH_EMBD_W);
        vision_model.patch_embeddings_b   = get_tensor(new_clip->ctx_data, TN_PATCH_EMBD_B);
        vision_model.class_embedding     = get_tensor(new_clip->ctx_data, TN_CLASS_EMBD);
        vision_model.position_embeddings = get_tensor(new_clip->ctx_data, format(TN_POS_EMBD, "v"));
        //vision_model.pre_ln_w            = get_tensor(new_clip->ctx_data, format(TN_LN_PRE, "v", "weight"));
        //vision_model.pre_ln_b            = get_tensor(new_clip->ctx_data, format(TN_LN_PRE, "v", "bias"));

        vision_model.linear_proj         = get_tensor(new_clip->ctx_data, TN_LINEAR_PROJ);
        vision_model.dense_h_to_4h       = get_tensor(new_clip->ctx_data, TN_DENSE_1);
        vision_model.gate_proj           = get_tensor(new_clip->ctx_data, TN_GATE_PROJ);
        vision_model.dense_4h_to_h       = get_tensor(new_clip->ctx_data, TN_DENSE_2);
        vision_model.boi_token           = get_tensor(new_clip->ctx_data, TN_BOI_TOKEN);
        vision_model.eoi_token           = get_tensor(new_clip->ctx_data, TN_EOI_TOKEN);
        vision_model.norm1_w             = get_tensor(new_clip->ctx_data, format(TN_PROJ_NORM, "weight"));
        vision_model.norm1_b             = get_tensor(new_clip->ctx_data, format(TN_PROJ_NORM, "bias"));
...
int clip_n_mmproj_embd(const struct clip_ctx * ctx) {
    return ctx->vision_model.norm1_w->ne[0];
}
